...
<header id="masthead" class="site-header site-head <?php if(get_header_image() && !is_single() ) : ?>full-height<?php endif; ?>" role="banner" <?php if(get_header_image() && !is_single() ) : ?>style="background-image: url(<?php header_image(); ?>);"<?php endif ?> itemscope itemtype="http://schema.org/WPHeader">
    <?php // Show navigation menu on everything except Single pages, unless Show Primary Nav Menu on Single Pages is enabled ?>
    <?php if ( !is_single() && independent_publisher_show_fix_nav_on_home() ) : ?>
        <nav role="navigation" class="fixed-nav overlay clearfix">
            ...
            <?php independent_publisher_fixed_nav(); ?>
        </nav>
    <?php elseif ( ! is_single() || independent_publisher_show_nav_on_single() ) : ?>
